...
function parseArgs() {
  ...
  const result = { clientes: 100, productos: 500, facturas: 1000 };
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a.startsWith('--')) {
      const [k, v] = a.includes('=') ? a.split('=') : [a, args[i+1]];
      if (k === '--clientes' || k === '-c') { result.clientes = parseInt(v, 10); if (!a.includes('=')) i++; }
      if (k === '--productos' || k === '-p') { result.productos = parseInt(v, 10); if (!a.includes('=')) i++; }
      if (k === '--facturas' || k === '-f') { result.facturas = parseInt(v, 10); if (!a.includes('=')) i++; }
    }
  }
  return result;
}
